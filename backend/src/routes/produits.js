...
router.post("/", auth, upload.fields([
  ...
  // Couleurs avec photos optionnelles
  if (couleurs) {
    const cols = JSON.parse(couleurs);
    for (let i = 0; i < cols.length; i++) {
      let couleur_image_url = null;
      if (req.files?.couleur_images?.[i]) {
        couleur_image_url = await uploadToCloudinary(req.files.couleur_images[i]);
      }
